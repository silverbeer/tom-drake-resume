...
class BuilderFactory:
    ...
    def create_builder(
        ...
        try:
            ...
            raise BuilderError(
                f"Failed to create {format_type} builder: {e}",
    ...
    def register_builder(cls, format_type: str, builder_class: Type[BaseBuilder]) -> None:
        ...
        logger.info(f"Registered builder for format '{format_type}': {builder_class.__name__}")
    ...
    def unregister_builder(cls, format_type: str) -> bool:
        ...
        if format_type in cls._builders:
            ...
            logger.info(f"Unregistered builder for format '{format_type}': {builder_class.__name__}")
            return True
        return False
    ...
    def get_builder_info(cls) -> Dict[str, Dict[str, str]]:
        ...
        for format_type, builder_class in cls._builders.items():
            info[format_type] = {
                ...
                "description": getattr(builder_class, "__doc__", "").split('\n')[0].strip()
            }
        return info
...
def create_builder(format_type: str, config: Config, theme: str = "modern") -> BaseBuilder:
