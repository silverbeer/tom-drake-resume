...
def _validate_business_rules(resume_data: ResumeData) -> tuple[List[str], List[str]]:
    ...
    # Skills validation
    total_skills = sum(len(cat.skills) for cat in resume_data.skills.categories.values())
    if total_skills < 10:
        warnings.append(f"Only {total_skills} skills listed - consider adding more for ATS optimization")
    elif total_skills > 50:
        warnings.append(f"{total_skills} skills listed - consider focusing on top skills for readability")
    ...
    for exp in resume_data.experience:
        for achievement in exp.achievements:
            if not achievement.ai_enhanced:
                unenhanced_achievements.append(f"{exp.company}: {achievement.description[:50]}...")
    ...
    for exp in resume_data.experience:
        for achievement in exp.achievements:
            if not achievement.metrics:
                achievements_without_metrics.append(f"{exp.company}: {achievement.description[:50]}...")
