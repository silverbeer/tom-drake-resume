...
class Config(BaseSettings):
    ...
    resume_file: Path = Field(
        default="resume.yml", description="Path to resume YAML file"
    )
    schema_file: Path = Field(
        default="schemas/resume-schema.json", description="Path to JSON schema"
    )
    output_dir: Path = Field(
        default="dist", description="Output directory for built files"
    )
    ...
    claude_api_key: str | None = Field(
        default=None, description="Claude API key for AI features"
    )
    claude_model: str = Field(
        default="claude-3-sonnet-20240229", description="Claude model to use"
    )
    ...
    openai_api_key: str | None = Field(
        default=None, description="OpenAI API key as fallback"
    )
    ...
    log_format: str = Field(
        default="structured", description="Log format (structured, plain)"
    )
    ...
    def validate_pdf_engine(cls, v):
        ...
        if v not in supported_engines:
            raise ValueError(
                f"Unsupported PDF engine: {v}. Supported: {supported_engines}"
            )
        return v
